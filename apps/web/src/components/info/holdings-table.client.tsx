...
function CategoryCard({
  ...
  return (
    <div ref={scrollTargetRef} className="rounded-lg border border-border overflow-hidden scroll-mt-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start gap-6 p-4">
        <div className="w-56 h-56 shrink-0 self-center sm:self-auto">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={chartData} cx="50%" cy="50%" innerRadius={60} outerRadius={100} dataKey="value" strokeWidth={0}>
                {chartData.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={entry.color} />
                ))}
              </Pie>
              <Tooltip formatter={(value) => formatCurrency(value as number)} contentStyle={chartTooltipStyle} />
            </PieChart>
          </ResponsiveContainer>
        </div>
    ...
    <div className="text-sm">
      {isClickable ? (
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="text-right shrink-0 tabular-nums">
            {holding.amount ? (
              <AmountDisplay amount={holding.amount} weight="semibold" percentage={ratio} percentageDecimals={1} fixedWidth />
            ) : "-"}
          </div>
        </div>
      )}
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm sm:grid-cols-3 lg:grid-cols-4">
            {holding.unrealizedGain !== null && (
              <div className="flex flex-col">
                ...
                <AmountDisplay amount={holding.unrealizedGain} type="balance" showSign size="sm" className="tabular-nums" />
              </div>
            )}
            {holding.unrealizedGainPct !== null && (
              <div className="flex flex-col">
                <span className="text-muted-foreground">評価損益率</span>
                <span className={cn("font-medium tabular-nums", getAmountColorClass({ value: holding.unrealizedGainPct, type: "balance" }))}>
            ...
            {holding.dailyChange !== null && (
              <div className="flex flex-col">
                ...
                <AmountDisplay amount={holding.dailyChange} type="balance" showSign size="sm" className="tabular-nums" />
              </div>
            )}
      ...
      {isClickable && isExpanded && (
        <div className="pl-[18px] pt-1.5">
          {hasMainDetails ? (
            <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm sm:grid-cols-3 lg:grid-cols-4">
              {holding.avgCostPrice !== null && <DetailItem label="平均取得単価" value={holding.avgCostPrice.toLocaleString()} />}
              {holding.unitPrice !== null && <DetailItem label="現在単価" value={holding.unitPrice.toLocaleString()} />}
              {holding.quantity !== null && <DetailItem label="数量" value={holding.quantity.toLocaleString()} />}
              {!hideAccountName && holding.accountName && <DetailItem label="保有金融機関" value={holding.accountName} truncate />}
            </div>
          ) : (
...
function DetailItem({ label, value, valueClassName, truncate }: { label: string; value: string; valueClassName?: string; truncate?: boolean }) {
  return (
    <div className={cn("flex flex-col", truncate && "min-w-0")}>
      <span className="text-muted-foreground">{label}</span>
      <span className={cn("font-medium tabular-nums", truncate && "truncate", valueClassName)}>{value}</span>
    </div>
  );
}
