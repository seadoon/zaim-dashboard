...
function groupSmallHoldings(
  ...
  for (const h of sorted) {
    ...
    if (ratio >= threshold) {
      result.push({ name: h.name, value: h.amount, color: getGainColor(h.unrealizedGain) });
    } else {
  ...
  if (othersValue > 0) {
    result.push({ name: "その他", value: othersValue, color: getGainColor(othersGain) });
  }
...
export function UnrealizedGainCardClient({
  ...
  const filteredHoldings = useMemo(() => {
    if (selectedFilter === ALL_FILTER) return holdings;
    if (selectedFilter.includes("|")) {
      const [institution, categoryName] = selectedFilter.split("|");
      return holdings.filter((h) => h.institution === institution && h.categoryName === categoryName);
    }
  ...
  const topLosers = sortedByGain.filter((h) => h.unrealizedGain < 0).slice(-3).reverse();
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-3 gap-4 text-xs sm:text-sm">
          <div>
            ...
            <AmountDisplay amount={totalGain} type="balance" size="lg" weight="bold" className="text-base sm:text-lg" />
          </div>
          <div>
                ...
                <div className="space-y-1.5">
                  {topGainers.map((h, i) => (
                    <div key={`${h.name}-${i}`} className="flex items-center text-sm">
                      ...
                      <AmountDisplay amount={h.unrealizedGain} type="balance" showSign size="sm" percentage={h.unrealizedGainPct ?? undefined} fixedWidth className="shrink-0" />
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-1.5">
                  {topLosers.map((h, i) => (
                    <div key={`${h.name}-${i}`} className="flex items-center text-sm">
                      <span className="truncate min-w-0 flex-1">{h.name}</span>
                      <AmountDisplay amount={h.unrealizedGain} type="balance" size="sm" percentage={h.unrealizedGainPct ?? undefined} fixedWidth className="shrink-0" />
                    </div>
                  ))}
                </div>
              </div>
